package me.bugsyftw.heroes.player.data.uuid;
...
public class UUIDManager {
	 ...
	 * This method has to run async, because it will use the lookup from the Mojang API. <br>
	 * It also takes care of already cached values. It doesn't lookup new players when it still has old, valid ones stored.
	 * 
	 * @param names
	...
	/**
	 * Get the Minecraft name of the player that is hooked to this Mojang account UUID. <br>
	...
	private static boolean shouldUpdateValue(String playerName) {
		...
		if (!isLastCached(playerName) || !isCachedUUID(playerName)) return true;
	...
	private static long getLastCached(String playerName) {
		// Already found
		if (lastCached.containsKey(playerName)) return lastCached.get(playerName);
	...
	public static void removeCachedPlayer(Player player) {
		...
		if (!isCachedUUID(player.getName())) return;
