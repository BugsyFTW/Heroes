...
public class BlockProjectile extends EntityFallingBlock implements CustomProjectile, IProjectile {
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private List<TypedRunnable<BlockProjectile>> typedRunnables = new ArrayList();
	...
	public void h() {
		...
		if (movingobjectposition != null) {
			if ((movingobjectposition.type == EnumMovingObjectType.BLOCK) && (!isIgnored(this.world.getWorld().getBlockAt(movingobjectposition.b, movingobjectposition.c, movingobjectposition.d).getType()))) {
				event = new BlockProjectileHitEvent(this, this.world.getWorld().getBlockAt(movingobjectposition.b, movingobjectposition.c, movingobjectposition.d), CraftBlock.notchToBlockFace(movingobjectposition.face), getMaterial(), getData());
				Bukkit.getPluginManager().callEvent(event);
				if (!event.isCancelled()) {
					ParticleEffect.displayBlockCrack(getBukkitEntity().getLocation(), Block.b(this.id), (byte) 0, 0.0F, 0.0F, 0.0F, 1.0F, 60);
					die();
				}
				...
				event = new BlockProjectileHitEvent(this, living, getMaterial(), getData());
				Bukkit.getPluginManager().callEvent(event);
				if (!event.isCancelled()) {
					ParticleEffect.displayBlockCrack(getBukkitEntity().getLocation(), Block.b(this.id), (byte) 0, 0.0F, 0.0F, 0.0F, 1.0F, 60);
					die();
			...
			event = new BlockProjectileHitEvent(this, getBukkitEntity().getLocation().getBlock().getRelative(BlockFace.DOWN), BlockFace.UP, getMaterial(), getData());
			Bukkit.getPluginManager().callEvent(event);
			if (!event.isCancelled()) {
				ParticleEffect.displayBlockCrack(getBukkitEntity().getLocation(), Block.b(this.id), (byte) 0, 0.0F, 0.0F, 0.0F, 1.0F, 60);
				die();
		...
		if (isAlive()) {
			...
			for (TypedRunnable<BlockProjectile> r : this.typedRunnables) {
				r.run(this);
			}
		}
	}
	...
	public Material getMaterial() {
		return Material.getMaterial(Block.b(this.id));
	}
	...
	public void addTypedRunnable(TypedRunnable<? extends CustomProjectile> r) {
		this.typedRunnables.add((TypedRunnable<BlockProjectile>) r);
	}
